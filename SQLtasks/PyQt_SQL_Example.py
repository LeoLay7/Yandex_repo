...
def a():
    with sq.connect('music_db.sqlite') as connection:
        cur = connection.cursor()
        genre = str(input())
        cur.execute(f"""SELECT DISTINCT Album.Title FROM Album WHERE AlbumId in (SELECT DISTINCT
         t.AlbumId FROM Track t WHERE GenreId ==
          (SELECT g.GenreId FROM Genre g WHERE g.Name == '{genre}'))""")
        res = cur.fetchall()
        # print(res)
        for i in res:
            print(*i)
